    ...
    <el-row class="content">
      <el-col :span="24" class="search">
        <el-form :inline="true" :model="SearchData" ref="SearchData" class="demo-form-inline">
          <el-form-item label="品类" prop="grainId">
            <el-select v-model="SearchData.grainId" placeholder="请选择"  @change="GetgrainVarietylist">
      ...
      <el-col :span="24" class="list">
        <el-table :data="tableData" width="100%" v-loading="dataloading" element-loading-text="数据加载中" element-loading-spinner="el-icon-loading" :header-cell-style="{background:'#EBECED'}">
          ...
          <el-table-column show-overflow-tooltip label="价格(元/吨)" align="center"><template slot-scope="{row}">{{ row.orderBugprice || '0' }}</template></el-table-column>
          <el-table-column show-overflow-tooltip label="合同数量(吨)" align="center"><template slot-scope="{row}">{{ row.contractNumber || '0' }}</template></el-table-column>
          <el-table-column show-overflow-tooltip label="完成数量(吨)" align="center"><template slot-scope="{row}">{{ row.finishNumbers || '0' }}</template></el-table-column>
          ...
          <el-table-column show-overflow-tooltip label="实际存粮地点" align="center"><template slot-scope="{row}">{{ row.storageName || '--' }}</template></el-table-column>
    ...
    methods:{
      ...mapActions([
        ...
        GetList:function(){
          ...
          AdminApi.getMyForwardTaskByOrderContract(Params).then(data=>{
        ...
        GetInfo:function(row){
          ...
          this.$router.push({path:'/Salestask/forwardlist/forwardinfo'})
        },
